...
# Script para actualizar PDF Validator API desde Git
...
echo -e "${CYAN}"
echo "═══════════════════════════════════════════════════════════════════"
echo "                PDF VALIDATOR API - ACTUALIZACIÓN DESDE GIT         "
...
UPDATE_DIR="/opt/pdf-validator-update"
...
check_tomcat_installation() {
    ...
    if [ ! -d "$TOMCAT_HOME" ]; then
        error "Tomcat no está instalado en $TOMCAT_HOME. Ejecuta primero deploy-from-git.sh"
    fi
...
get_repository_url() {
    ...
    # Intentar leer configuración anterior
    if [ -f "/opt/pdf-validator-git-config" ]; then
        source /opt/pdf-validator-git-config
        info "Configuración anterior encontrada:"
        echo -e "${GREEN}  Repositorio: $GIT_REPO${NC}"
        echo -e "${GREEN}  Rama: $GIT_BRANCH${NC}"
        
        if confirm "¿Usar la misma configuración?"; then
            return
        fi
    fi
    ...
    echo "GIT_REPO=\"$GIT_REPO\"" > /opt/pdf-validator-git-config
    echo "GIT_BRANCH=\"$GIT_BRANCH\"" >> /opt/pdf-validator-git-config
}
...
# Función para descargar y compilar
download_and_compile() {
    log "Descargando última versión desde Git..."
    ...
    success "Repositorio descargado exitosamente"
...
deploy_new_version() {
    ...
    # Copiar nuevo WAR
    cp "$UPDATE_DIR/pdf-validator-source/target/$WAR_FILE" "$WEBAPPS_DIR/"
...
main_update() {
    log "Iniciando actualización desde Git..."
    ...
    echo -e "${GREEN}  ✓ Descargará la última versión desde: $GIT_REPO${NC}"
...
echo "Actualización desde Git completada exitosamente en $(date)" >> /opt/deployment-history.log
...
success "¡Actualización desde Git completada exitosamente!"
exit 0